...
class ImageCrop(Node):
    def __init__(self):
        ...
        # 크롭 영역 설정 (280, 230)에서 (220x120 크기)
        self.crop_x = 280
        self.crop_y = 230
        self.crop_width = 220
        self.crop_height = 120
    ...
    def color_callback(self, msg):
        ...
        # 크롭 영역 추출
        cropped_color_image = resized_color_image[self.crop_y:self.crop_y + self.crop_height,
                                                  self.crop_x:self.crop_x + self.crop_width]

        # OpenCV로 컬러 이미지 및 크롭된 이미지 시각화
        cv2.imshow("Resized Color Image", resized_color_image)
        cv2.imshow("Cropped Color Image", cropped_color_image)
        cv2.waitKey(1)
    ...
    def depth_callback(self, msg):
        ...
        # 크롭 영역 추출
        cropped_depth_image = resized_depth_image[self.crop_y:self.crop_y + self.crop_height,
                                                  self.crop_x:self.crop_x + self.crop_width]

        # 8비트로 변환 후 시각화
        depth_image_8u = np.uint8(resized_depth_image)
        cropped_depth_image_8u = np.uint8(cropped_depth_image)

        # OpenCV로 깊이 이미지 및 크롭된 이미지 시각화
        cv2.imshow("Resized Depth Image", depth_image_8u)
        cv2.imshow("Cropped Depth Image", cropped_depth_image_8u)
        cv2.waitKey(1)

def main(args=None):
    rclpy.init(args=args)

    image_crop_node = ImageCrop()

    try:
        rclpy.spin(image_crop_node)
    except KeyboardInterrupt:
        image_crop_node.get_logger().info("Node stopped cleanly")
    except Exception as e:
        image_crop_node.get_logger().error(f"Error: {str(e)}")
    finally:
        image_crop_node.destroy_node()
        rclpy.shutdown()
