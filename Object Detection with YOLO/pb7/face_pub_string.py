...
class YOLOv5ObjectDetection(Node):
    def __init__(self):
        ...
        self.publisher = self.create_publisher(
            String,
            '/object_name',  # 객체 이름을 퍼블리시할 토픽 이름
            10
        )
    ...
    def image_callback(self, msg):
        ...
        # 검출된 객체 이름을 문자열로 생성
        detected_objects = results.pandas().xyxy[0]['name'].unique()  # 검출된 객체 이름 목록 추출
        object_names = ", ".join(detected_objects)  # 객체 이름을 쉼표로 구분하여 하나의 문자열로 만듦

        # 객체 이름을 String 메시지로 퍼블리시
        object_msg = String()
        object_msg.data = object_names
